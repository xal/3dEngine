...
import com.custom_3DEnjine.drawing_utils.FaceDrawer;
...
public abstract class Primitive {
    ...
    */
	...
	protected void initializeGeometry() {
	    ...
	    //initialize drawers
	    drawers = new FaceDrawer[faces.length + 2];
		for (int i = 0; i < faces.length + 2; i++) 
			drawers[i] = new FaceDrawer();
	...
	public void setEnabled(boolean isEnabled) {
		...
		if (isEnabled)
    		setBackgroundColor(Constants.BACKGROUND_ENABLED_COLOR);
    	else
    		setBackgroundColor(Color.TRANSPARENT);
	}
     ...
     */
    public void setFacesColor(int color, int startIndex, int endIndex) {
        Paint paint = new Paint();
        paint.setColor(color);
        paint.setStyle(Style.FILL);
